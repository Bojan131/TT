...
class Base{
    ...
    async deactivateActivateTicker(number, state) {
        ...
        for (let i = 0; i < count; ++i) {
            ...
            const shouldActivate = state === "on" && text.includes("Activate");
            const shouldDeactivate = state === "off" && text.includes("Deactivate");
    
            if (shouldActivate || shouldDeactivate) {
                await tickerDrawer.nth(actionIndex).click();
                await this.page.waitForTimeout(2000);
                if (state === "on" && text.includes("Deactivate")) {
                    await tickerDrawer.nth(actionIndex).click();
                } else if (state === "off" && text.includes("Activate")) {
                    await tickerDrawer.nth(actionIndex).click();
                }
                break;
            }
