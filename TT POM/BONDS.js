...
class BONDS{
    constructor(page){
        ...
        this.bondsSerachFilters = page.locator(".page-content .display-string");
        ...
        this.bondsSerachTable = page.locator(".bonds_search_table_wrapper");
        this.checkmark = page.locator(".page-content .checkmark");
        this.bondsIpoSerachOptions = page.locator(".bonds_ipo_options .display-string");
    ...
    async bonds(){
        ...
        await this.base.isActive("Yield Curves");
        const expectedTexts = [
            "Yield Curves", "Historic Yield Curves","Yield curves comparison", "Bond spreads"
          ];
          await this.base.checkItems(this.tabItem,expectedTexts);
          await expect(this.highChart).toBeVisible();
          const expectedTexts2 = [
            "Yields", "Government Bonds","Government Bond Yields"
          ];
          await this.base.checkItems(this.tableHeaderName,expectedTexts2);
    }
    ...
    async selectYieldCountry(){
        ...
        const expectedTexts = [
          ...
          await this.base.checkItems(this.yieldCountryName,expectedTexts);
...
await this.yieldCheckbox.nth(17).click();
    ...
    async historicYieldCurves(){
        ...
        const expectedTexts = [
            "US Government Bond Yields","German Government Bond Yields","Yield comparison (today)"
          ];
          await this.base.checkItems(this.highChartTitle,expectedTexts);
    }

    async bondsSerach(){

        await expect(this.bondsSerachFilters.last()).toBeVisible();
        await this.bondsSerachFilters.first().click();
        await this.base.chooseEllipsis("Countries and Regions");
        await this.bondsSerachFilters.nth(3).click();
        await this.base.chooseEllipsis("Commercial Papers");
        await this.bondsSerachFilters.nth(3).click();
        await this.checkmark.last().click();
        await this.maturityFrom.fill("0");
        await this.maturityTo.fill("0.5");
        await this.submitButton.click();
        await expect(this.bondsSerachTable).toBeVisible();
    }

    async bondsIpo(){

        await this.page.waitForSelector(".bonds_ipo_options", { state: 'visible'});
        const countFilter = await this.bondsIpoSerachOptions.count();
        expect(countFilter).toEqual(5);
        await this.bondsIpoSerachOptions.first().click();
        await this.base.chooseEllipsis("Corporates");
        await this.page.waitForSelector(".page-content .loader_wrapper", { state: 'hidden' });
        await this.bondsIpoSerachOptions.nth(1).click();
        await this.base.chooseEllipsis("Germany");
        await this.page.waitForSelector(".page-content .loader_wrapper", { state: 'hidden' });
        await this.bondsIpoSerachOptions.nth(2).click();
        await this.base.chooseEllipsis("Traditional Bonds");
        await this.page.waitForSelector(".page-content .loader_wrapper", { state: 'hidden' });
    }
