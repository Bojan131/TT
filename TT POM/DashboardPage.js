...
class DashboardPage extends Base {
  ...
  async hoverAll() {
    ...
    for (let i = 0; i < count; ++i) {
      ...
      const isIconVisible = await this.page
        .locator('.news_source_icon')
        .nth(i)
        .isVisible({ timeout: 3000 })
        .catch(() => false)
      if (!isIconVisible) {
        const noNewsText = await this.page
          .locator('.no_news_data')
  ...
  async dropdownTicker() {
    ...
    for (let i = 0; i <= 12; i++) {
      const a = await this.page.locator('.ellipsis-option').nth(i).textContent()
  ...
  async compareTicker(browser) {
    ...
    const [newPage] = await Promise.all([context.waitForEvent('page'), page.locator('.header-control').nth(2).click({ force: true })])
